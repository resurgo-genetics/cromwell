...
trait WdlStandardLibraryFunctions extends WdlFunctions[WomValue] {
  ...
  def stdout(params: Seq[Try[WomValue]]): Try[WdlFile]

  def stderr(params: Seq[Try[WomValue]]): Try[WdlFile]

  def glob(path: String, pattern: String): Seq[String]

  def size(params: Seq[Try[WomValue]]): Try[WdlFloat]

  private def writeContent(baseName: String, content: String): Try[WdlFile] = writeFile(s"${baseName}_${content.md5Sum}.tmp", content)

  private def writeToTsv[A <: WomValue with TsvSerializable](functionName: String, params: Seq[Try[WomValue]], defaultIfOptionalEmpty: A): Try[WdlFile] = {
    val wdlClass = defaultIfOptionalEmpty.getClass
    def castOrDefault(wdlValue: WomValue): A = wdlValue match {
  ...
  def read_objects(params: Seq[Try[WomValue]]): Try[WdlArray] = extractObjects("read_objects", params) map { WdlArray(WdlArrayType(WdlObjectType), _) }
  def read_string(params: Seq[Try[WomValue]]): Try[WdlString] = readContentsFromSingleFileParameter("read_string", params).map(s => WdlString(s.trim))
  def read_json(params: Seq[Try[WomValue]]): Try[WomValue] = Failure(new NotImplementedError(s"read_json() not implemented yet"))
  def read_int(params: Seq[Try[WomValue]]): Try[WdlInteger] = read_string(params) map { s => WdlInteger(s.value.trim.toInt) }
  def read_float(params: Seq[Try[WomValue]]): Try[WdlFloat] = read_string(params) map { s => WdlFloat(s.value.trim.toDouble) }

  def write_lines(params: Seq[Try[WomValue]]): Try[WdlFile] = writeToTsv("write_lines", params, WdlArray(WdlArrayType(WdlStringType), List.empty[WdlValue]))
  def write_map(params: Seq[Try[WomValue]]): Try[WdlFile] = writeToTsv("write_map", params, WdlMap(WdlMapType(WdlStringType, WdlStringType), Map.empty[WdlValue, WdlValue]))
  def write_object(params: Seq[Try[WomValue]]): Try[WdlFile] = writeToTsv("write_object", params, WdlObject(Map.empty[String, WdlValue]))
  def write_objects(params: Seq[Try[WomValue]]): Try[WdlFile] = writeToTsv("write_objects", params, WdlArray(WdlArrayType(WdlObjectType), List.empty[WdlObject]))
  def write_tsv(params: Seq[Try[WomValue]]): Try[WdlFile] = writeToTsv("write_tsv", params, WdlArray(WdlArrayType(WdlStringType), List.empty[WdlValue]))
  def write_json(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError(s"write_json() not implemented yet"))

  def read_lines(params: Seq[Try[WomValue]]): Try[WdlArray] = {
    for {
  ...
  def read_map(params: Seq[Try[WomValue]]): Try[WdlMap] = {
    for {
  ...
  def read_object(params: Seq[Try[WomValue]]): Try[WdlObject] = {
  ...
  def read_tsv(params: Seq[Try[WomValue]]): Try[WdlArray] = {
    for {
  ...
  def read_boolean(params: Seq[Try[WomValue]]): Try[WdlBoolean] = {
    read_string(params) map { s => WdlBoolean(java.lang.Boolean.parseBoolean(s.value.trim.toLowerCase)) }
  }

  def glob(params: Seq[Try[WomValue]]): Try[WdlArray] = {
    for {
  ...
  def basename(params: Seq[Try[WomValue]]): Try[WdlString] = {
  ...
  def floor(params: Seq[Try[WomValue]]): Try[WdlInteger] = {
  ...
  def round(params: Seq[Try[WomValue]]): Try[WdlInteger] = {
  ...
  def ceil(params: Seq[Try[WomValue]]): Try[WdlInteger] = {
  ...
  def transpose(params: Seq[Try[WomValue]]): Try[WdlArray] = {
    ...
    case class ExpandedTwoDimensionalArray(innerType: WdlType, value: Seq[Seq[WomValue]])
    def validateAndExpand(value: WomValue): Try[ExpandedTwoDimensionalArray] = value match {
      case WdlArray(WdlArrayType(WdlArrayType(innerType)), array: Seq[WomValue]) => expandWdlArray(array) map { ExpandedTwoDimensionalArray(innerType, _) }
  ...
  def length(params: Seq[Try[WomValue]]): Try[WdlInteger] = {
    ...
    def arrayLength(value: WomValue): Try[WdlInteger] = value match {
      case WdlArray(_, arrayValues) => Success(WdlInteger(arrayValues.length))
      case bad => Failure(new UnsupportedOperationException(s"length() expects one parameter of type Array but got one parameter of type ${bad.womType.toWdlString}"))
    }

    extractArguments flatMap arrayLength
  }

  def prefix(params: Seq[Try[WomValue]]): Try[WdlArray] = {
    ...
    val makePrefixedString = (prefixString: WomValue, elements: WomValue) => (prefixString, elements) match {
      ...
      case (_, _) =>
        Failure(new UnsupportedOperationException(s"The function prefix expect arguments (String, Array[X]) where X is a primitive type, but got (${prefixString.womType.toWdlString}, ${elements.womType.toWdlString})"))
    }

    extractTwoArguments flatMap makePrefixedString.tupled
  }

  def range(params: Seq[Try[WomValue]]): Try[WdlArray] = {
    ...
    def validateArguments(value: Try[WomValue]) = value match {
      case Success(intValue: WomValue) if WdlIntegerType.isCoerceableFrom(intValue.womType) =>
  ...
  def sub(params: Seq[Try[WomValue]]): Try[WdlString] = {
    ...
    def validateArguments(values: (Try[WomValue], Try[WomValue], Try[WomValue])) = values match {
      case (Success(strValue), Success(WdlString(pattern)), Success(replaceValue))
        if WdlStringType.isCoerceableFrom(strValue.womType) &&
          WdlStringType.isCoerceableFrom(replaceValue.womType) =>
  ...
  def select_first(params: Seq[Try[WomValue]]): Try[WomValue] = extractSingleArgument("select_first", params) flatMap {
    case WdlArray(WdlArrayType(WdlOptionalType(memberType)), arrayValue) =>
      if (arrayValue.isEmpty) SelectFirstEmptyInput else (arrayValue collectFirst {
        ...
        case wdlValue if memberType.isCoerceableFrom(wdlValue.womType) =>
    ...
    case other => Failure(new IllegalArgumentException(s"select_first must take an array but got ${other.womType.toWdlString}: ${other.toWdlString}"))
  }

  def select_all(params: Seq[Try[WomValue]]): Try[WdlArray] = extractSingleArgument("select_all", params) flatMap {
    case WdlArray(WdlArrayType(WdlOptionalType(memberType)), arrayValue) =>
      Success(WdlArray(WdlArrayType(memberType), arrayValue collect {
        case WdlOptionalValue(_, Some(wdlValue)) => wdlValue
        case wdlValue if memberType.isCoerceableFrom(wdlValue.womType) => memberType.coerceRawValue(wdlValue).get
      }))
  ...
  def defined(params: Seq[Try[WomValue]]): Try[WdlBoolean] = extractSingleArgument("defined", params) map {
    case WdlOptionalValue(_, None) => WdlBoolean(false)
    case _ => WdlBoolean(true)
  }

  def zip(params: Seq[Try[WomValue]]): Try[WdlArray] = {
  ...
  def cross(params: Seq[Try[WomValue]]): Try[WdlArray] = {
  ...
  private def extractObjects(functionName: String, params: Seq[Try[WomValue]]): Try[Array[WdlObject]] = for {
...
object WdlStandardLibraryFunctions {
  def fromIoFunctionSet(ioFunctionSet: IoFunctionSet) = new WdlStandardLibraryFunctions {
    ...
    override def stdout(params: Seq[Try[WomValue]]): Try[WdlFile] = ioFunctionSet.stdout(params)

    override def stderr(params: Seq[Try[WomValue]]): Try[WdlFile] = ioFunctionSet.stderr(params)

    override def glob(path: String, pattern: String): Seq[String] = ioFunctionSet.glob(path, pattern)

    override def size(params: Seq[Try[WomValue]]): Try[WdlFloat] = ioFunctionSet.size(params)
  }
  ...
  def assertEquallySizedArrays[A](values: (WomValue, WomValue), badArgsFailure: Failure[Nothing] ): Try[(WdlArray, WdlArray)] = values match {
  ...
  def assertArrays(values: (WomValue, WomValue), badArgsFailure: Failure[Nothing] ): Try[(WdlArray, WdlArray)] = values match {
...
trait PureStandardLibraryFunctionsLike extends WdlStandardLibraryFunctions {
  ...
  override def write_json(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError(s"write_json not available in $className."))
  override def size(params: Seq[Try[WomValue]]): Try[WdlFloat] = Failure(new NotImplementedError(s"size not available in $className."))
  override def write_tsv(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError(s"write_tsv not available in $className."))
  override def stdout(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError(s"stdout not available in $className."))
  override def glob(path: String, pattern: String): Seq[String] = throw new NotImplementedError(s"glob not available in $className.")
  override def stderr(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError(s"stderr not available in $className."))
}
...
class WdlStandardLibraryFunctionsType extends WdlFunctions[WdlType] {
  ...
  def size(params: Seq[Try[WdlType]]): Try[WdlType] = {
    def isGoodFirstSizeParam(womType: WdlType): Boolean = womType match {
...
case object NoFunctions extends WdlStandardLibraryFunctions {
  ...
  override def stdout(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError())
  override def stderr(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError())
  override def read_json(params: Seq[Try[WomValue]]): Try[WomValue] = Failure(new NotImplementedError())
  override def write_tsv(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError())
  override def write_json(params: Seq[Try[WomValue]]): Try[WdlFile] = Failure(new NotImplementedError())
  override def size(params: Seq[Try[WomValue]]): Try[WdlFloat] = Failure(new NotImplementedError())
  override def length(params: Seq[Try[WomValue]]): Try[WdlInteger] = Failure(new NotImplementedError())
  override def sub(params: Seq[Try[WomValue]]): Try[WdlString] = Failure(new NotImplementedError())
  override def range(params: Seq[Try[WomValue]]): Try[WdlArray] = Failure(new NotImplementedError())
  override def transpose(params: Seq[Try[WomValue]]): Try[WdlArray] = Failure(new NotImplementedError())
  override def select_first(params: Seq[Try[WomValue]]): Try[WdlArray] = Failure(new NotImplementedError())
  override def select_all(params: Seq[Try[WomValue]]): Try[WdlArray] = Failure(new NotImplementedError())
  override def zip(params: Seq[Try[WomValue]]): Try[WdlArray] = Failure(new NotImplementedError())
  override def cross(params: Seq[Try[WomValue]]): Try[WdlArray] = Failure(new NotImplementedError())
  override def basename(params: Seq[Try[WomValue]]): Try[WdlString] = Failure(new NotImplementedError())
  override def floor(params: Seq[Try[WomValue]]): Try[WdlInteger] = Failure(new NotImplementedError())
  override def round(params: Seq[Try[WomValue]]): Try[WdlInteger] = Failure(new NotImplementedError())
  override def ceil(params: Seq[Try[WomValue]]): Try[WdlInteger] = Failure(new NotImplementedError())
}
